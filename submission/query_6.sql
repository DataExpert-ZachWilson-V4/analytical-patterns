...
-- CTE to calculate wins for each game
game_wins AS (
    SELECT
        gd.team_id,
        gd.game_id,
...
-- CTE to calculate cummulative wins over a 90 game sliding window
cummulative_team_wins AS (
    SELECT
        ...
        team_abbreviation,
        game_date_est,
        SUM(win) OVER (PARTITION BY team_id ORDER BY game_date_est ROWS BETWEEN 89 PRECEDING AND CURRENT ROW) AS rolling_90_game_wins
    FROM
        game_wins
),
