...
WITH
    ...
    -- Filter to get player-season combinations excluding overall totals
    nba_game_details_dashboard_player_season_total AS (
        SELECT
            *
        FROM
...
-- Select the player with the highest total points and the corresponding season
SELECT
    player,
    MAX(total_points) AS max_total_points,
    MAX_BY(season, total_points) AS season
FROM
    nba_game_details_dashboard_player_season_total
GROUP BY
    player
ORDER BY
