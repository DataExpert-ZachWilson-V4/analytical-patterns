CREATE
OR REPLACE TABLE mposada.hw_5_q_2 AS
WITH
...
SELECT
  ...
  SUM(
    CASE
      WHEN (
        dgd.team_id = dng.home_team_id
        AND dng.home_team_wins = 1
      )
      OR (
        dgd.team_id = dng.visitor_team_id
        AND dng.home_team_wins = 0
      ) THEN 1
      ELSE 0
    END
  ) AS total_wins
FROM
  ...
  LEFT JOIN deduplicated_nba_games dng ON dgd.game_id = dng.game_id -- Join deduplicated game details with deduplicated games
WHERE
  dgd.rn = 1
  AND dng.rn = 1 -- Only include the first row in each group
GROUP BY
  GROUPING SETS (
