WITH yesterday AS (
  -- nba_player_status is a table to store 5 categories of player_status
  SELECT *
...
, today AS (
  SELECT player_name
       -- boolean to detemine if the player is active in current season or not
       , is_active
       -- partitioning field to get current season
       , current_season
       -- get the latest season in which the player contributed by getting the first elemnt from seasons array
...
SELECT COALESCE(y.player_name, t.player_name) AS player_name
     ...
     -- player status determined by is_active flag, current and active season calculations
